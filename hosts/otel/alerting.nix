{
  ...
  alertRules = (pkgs.formats.yaml {}).generate "homelab-alerts.yml" {
    groups = [
      ...
      {
        # comin's own health. Every metric below was read off a live exporter on
        # 2026-09-08 rather than guessed -- the names are comin 0.14.0's actual
        # gauges, and a rule written against an invented name looks correct and
        # silently never fires.
        #
        # Why this group exists: comin on otel deployed NOTHING for 7.34 days
        # while comin.service sat `active`, polling happily. It was failing at
        # the eval stage every cycle. Nothing scraped comin, so the only trace
        # was an indirect one -- prometheus_config_last_reload_success_timestamp
        # drifting a week stale -- and it took a manual colmena apply to break
        # the deadlock. comin_last_eval_failed was 1 the whole time and would
        # have said so on day one.
        #
        # All are plain 0/1 gauges. `for` is generous throughout: comin polls
        # every 60-130s (see pollerPeriods in modules/comin.nix), so a single
        # bad cycle is normal and only a persistent one is news.
        name = "comin";
        rules = [
          {
            alert = "CominEvalFailing";
            expr = "comin_last_eval_failed == 1";
            for = "30m";
            labels.severity = "warning";
            annotations = {
              summary = "comin cannot evaluate the flake on {{ $labels.instance }}";
              description = "comin fetches fine but every eval fails, so this host silently stops applying commits while its service still looks healthy. This is the exact failure that stalled otel for 7.34 days. Past causes: host memory pressure during eval (check MemAvailable before assuming a code regression) and genuine flake errors. Needs `journalctl -u comin` on the host.";
            };
          }
          {
            ...
            labels.severity = "warning";
            annotations = {
              summary = "comin cannot build the system closure on {{ $labels.instance }}";
              description = "Evaluation succeeded but the build failed, so the host is pinned to its current generation. Check whether the closure builds elsewhere and whether the attic cache is reachable.";
            };
          }
          {
            alert = "CominDeploymentFailing";
            expr = "comin_last_deployment_failed == 1";
            for = "30m";
            labels.severity = "warning";
            annotations = {
              summary = "comin built a closure but failed to activate it on {{ $labels.instance }}";
              description = "The system was built but switching to it failed -- usually a failing systemd unit in the new generation. The host keeps running the old generation.";
            };
          }
          {
            # Longer `for` than the rest on purpose: on 2026-08-20 a burst of
            # simultaneous fetches across ~15 hosts caused transient DNS/forgejo
            # failures that resolved on their own (see docs/, and the staggered
            # pollerPeriods added in modules/comin.nix as the fix). An hour
            # separates that from forgejo actually being unreachable.
            ...
            annotations = {
              ...
              description = "This host has not seen new commits for an hour. Check that forgejo.homelab.local is reachable and its cert is valid from this host -- comin clones over HTTPS trusting step-ca.";
            };
          }
          {
            ...
            annotations = {
              ...
              description = "A suspended comin deploys nothing at all, which looks identical to a quiet fleet. Resume it, or remove the host from the comin job in ./configuration.nix if this is deliberate.";
            };
          }
        ];
      }
